...
export class ProductsService {
  ...
  async initializeProduct(
    productUrl: string,
  ): Promise<{ sizes: ProductSizeAvailability[]; name: string; url: string }> {
  ...
  async addProduct(
    ...
    const duplicateSearchOptions = {
      url: product.url,
      'size.id': product.size.id,
      userId,
    };
    if (!!(await this.productRepository.findOne(duplicateSearchOptions))) {
      throw new ConflictException(
        'This product has already been added in the given size.',
      );
    }

    const newProduct: ProductEntity = this.productRepository.create(product);
    newProduct.userId = userId;
    const firstUpdate = await this.crawlerService.getUpdateData(
      product.url,
      product.size.id,
    );
  ...
  private async updateProduct(
    ...
    try {
      ...
      const newUpdate = await this.crawlerService.getUpdateData(
        product.url,
        sizeId,
      );
      const productAttributeChanges = this.getProductAttributeChanges(
        product,
        newUpdate,
      );
      ...
      if (error instanceof NotFoundException) {
        ...
        this.logger.error(
          {
            message:
              'Failed to update product more than 2 times. Set to inactive.',
            error: message,
            product,
          },
          error.stack,
  ...
  private getProductAttributeChanges(
    product: ProductEntity,
    ...
    if (product.isAvailable !== update.isAvailable) {
      const latestUpdateWhenAvailable = product.updates
        .slice()
        .reverse()
        .find(p => p.isAvailable);
      changesBuilder.setAvailabilityChange(!latestUpdateWhenAvailable);
      if (
        latestUpdateWhenAvailable &&
        latestUpdateWhenAvailable.price !== update.price
      ) {
        changesBuilder.setPriceChange({
    ...
    if (product.isLowInStock !== update.isLowInStock) {
      const latestUpdateWhenLowInStock = product.updates.find(
        p => p.isLowInStock,
      );
  ...
  async setFavorite(
    ...
    if (isFavorite && favorites.length >= 3) {
      throw new ForbiddenException(
        'You have exceeded your favorites limit of three.',
      );
    }
  ...
  async deleteProductAsAdmin(productId: ObjectID): Promise<boolean> {
    ...
    return this.productRepository.delete({ _id: productId }).then(() => true);
  }
