...
username=${username}`;
  ...
  it('should reject if hash is not valid', async () => {
    await expect(
      service.login(createLoginData({ hash: 'wrong' })),
  ...
  it('should reject if auth_date is too old', async () => {
    await expect(
      service.login(
        createLoginData({ auth_date: Date.now() / 1000 - 10 * 60 }),
      ),
    ).rejects.toThrow(UnauthorizedException);
  });
  ...
  it('should authenticate existing user', async () => {
    ...
    await expect(await service.login(createLoginData({}))).toEqual({
      token: 'token',
      user: 'user',
    });
  ...
  it('should create a new user', async () => {
    ...
    usersService.createUserWithUniqueName.mockReturnValueOnce({
      _id: 'userid',
    });
    authService.createToken.mockReturnValueOnce({
      token: 'token',
